...
function datePicker({disabledDates,minDt,maxDt}){
  console.log('disabledDates >>>>>>>>>>', disabledDates)
    ...
    $("#start-date").datepicker("option", "beforeShowDay", function(date) {
  ...
  start.setDate(start.getDate()+1)
    $("#end-date").datepicker("option", "minDate", start);
    $('#start-date').datepicker("option", "defaultDate", startDate);

    localStorage.setItem('date_start_room', startDate)

    const inputDate = new Date(startDate).toISOString().split('T')[0]; // Change this to your desired input date

    const filteredDates = dates.filter(date => new Date(date) > new Date(inputDate));
    const firstFilteredDate = filteredDates.length > 0 ? filteredDates[0] : null;
    $('#end-date').datepicker('option', 'beforeShowDay', function(date) {
...
function parseData() {
var itemLocal = localStorage.getItem('Toclose')
datePicker(JSON.parse(itemLocal))
}
function parseDataGlobal() {
var itemLocalGlobal = localStorage.getItem('TocloseGlobal')
datePickerGlobal(JSON.parse(itemLocalGlobal))
}
